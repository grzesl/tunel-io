...
err_t tcp_server_send_data(void *arg)
{
    TCP_SERVER_T *state = (TCP_SERVER_T*)arg;
    ...
    if(state->to_send_len <= 0){
        return ERR_OK;
    }

    //for(int i=0; i< data_len; i++) {
    //    state->buffer_sent[i] = data[i];
    //}
    ...
    err_t err = tcp_write(state->client_pcb, state->buffer_sent, state->to_send_len, TCP_WRITE_FLAG_COPY);
...
int tcp_server_write(void *arg, char*data, int data_len){
    ...
    if(state->to_send_len + data_len > sizeof(state->buffer_sent)){
        DEBUG_printf("Failed to tcp_server_write \n");
        return ERR_BUF;
    }

    memcpy(&state->buffer_sent[ state->to_send_len], data, data_len);

    cyw43_arch_lwip_begin();
    state->to_send_len +=  data_len;
    cyw43_arch_lwip_end();

  //  DEBUG_printf("tcp_server_write\n");
...
int tcp_server_read(void *arg, char*data, int data_len){
    ...
    if(state->recv_len > data_len){
        DEBUG_printf("Failed to tcp_server_read \n");
        return ERR_BUF;
    }
    
    memcpy(data, state->buffer_recv,  state->recv_len);

    cyw43_arch_lwip_begin();
    int data_recv = state->recv_len;
    state->recv_len = 0;
    cyw43_arch_lwip_end();
...
static err_t tcp_server_accept(void *arg, struct tcp_pcb *client_pcb, err_t err) {
    ...
    tcp_poll(client_pcb, tcp_server_poll, 0 /* POLL_TIME_S * 2*/);
